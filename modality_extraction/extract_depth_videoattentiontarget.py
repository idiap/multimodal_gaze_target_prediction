...
def run(input_path, output_path, model_path, model_type="large", optimize=True):
    ...
    # get folders
    folder_paths = glob.glob(os.path.join(input_path, "*/*"))
    num_folders = len(folder_paths)
    print(f'\n Found {num_folders} sub-folders. \n')

    for f_ind, folder_path in enumerate(folder_paths):

        print("  Sub-folder {} ({}/{})".format(os.path.basename(folder_path), f_ind + 1, num_folders))

        # Get images from folder
        img_names = glob.glob(os.path.join(folder_path, "*"))
        num_images = len(img_names)
        print(f'\n    Found {num_images} images in sub-folder {os.path.basename(folder_path)}.\n')
        ...
        for ind, img_name in enumerate(img_names):
            ...
            subfolder_path = os.path.join(output_path, '/'.join(folder_path.split('/')[-2:]))
