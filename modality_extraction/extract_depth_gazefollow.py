...
def run(input_path, output_path, model_path, model_type="large", optimize=True):
    ...
    # Get high-level folders (ie. train and test2)
    split_folders = list(os.walk(input_path))[0][1] # get dataset-split directories
    print(f'Found the following split folders: {split_folders}')
    
    for split in split_folders:
        print(f'\n Working on folder {split}')
        # Get sub-folders
        folders = glob.glob(os.path.join(input_path, split, "*"))
        num_folders = len(folders)
        print(f'\n Found {num_folders} sub-folders. \n')

        for f_ind, folder in enumerate(folders):

            print("  Sub-folder {} ({}/{})".format(os.path.basename(folder), f_ind + 1, num_folders))

            # Get images from folder
            img_names = glob.glob(os.path.join(folder, "*"))
            num_images = len(img_names)
            print(f'\n    Found {num_images} images in sub-folder {os.path.basename(folder)}.\n')

            for ind, img_name in enumerate(img_names):

                print("     + processing {} ({}/{})".format(os.path.basename(img_name), ind + 1, num_images))
                ...
                subfolder_path = os.path.join(output_path, split, os.path.basename(folder))
